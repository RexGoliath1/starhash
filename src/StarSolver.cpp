...
StarSolver::StarSolver(int max_contours, int max_points_per_contour,
                       fs::path output_path)
    : max_contours(max_contours),
      max_points_per_contour(max_points_per_contour), output_path(output_path) {
}
...
void StarSolver::get_gauss_centroids() {
  ...
  // TODO: I think this is still broken for the median or the image is actually
  // flat.
  ...
  /* randomly sample part of image, that are at least num_edge_pixels away from
   * the edge */
  int num_pixels = (width - num_edge_pixels) * (height - num_edge_pixels);
  assert(num_pixels > num_flat_pixels);
  std::sample(boxed_iterator{num_edge_pixels}, boxed_iterator{width},
              std::back_inserter(flat_col_samples), num_flat_pixels, rng);
  std::sample(boxed_iterator{num_edge_pixels}, boxed_iterator{height},
              std::back_inserter(flat_row_samples), num_flat_pixels, rng);
  ...
  for (int ii = 0; ii < num_flat_pixels; ii++) {
    ...
    float diff =
        flat_image.at<uchar>(row + 5, col + 5) - flat_image.at<uchar>(row, col);
    diffs[ii] = diff;

#ifdef DEBUG_FLATTEN_IMAGE
    std::cout << "Sampled col: " << row << ", row: " << col;
    std::cout << "  Pixel + 5: "
              << static_cast<int>(flat_image.at<uchar>(row + 5, col + 5));
    std::cout << "  Pixel: "
              << static_cast<int>(flat_image.at<uchar>(row, col));
    std::cout << "  Diff =  " << diffs[ii] << std::endl;
#endif
  }

  /* Identify outliers to remove for random sample by thresholding the residual
  ...
  for (int ii = 0; ii < num_flat_pixels; ii++) {
    std::cout << "Index: " << ii << "   Inlier: " << inliers[ii]
              << "  Diff: " << diffs[ii] << "  Median: " << median
              << "  Mean: " << mean_diff
  ...
  std::cout << "Flattened Image Stddev with outliers: " << stddev_norm
            << std::endl;
#endif
...
double StarSolver::get_median(cv::Mat input, int n) {
  ...
  cv::Mat hist;
  cv::calcHist(&input, 1, 0, cv::Mat(), hist, 1, &n, &hist_range, uniform,
               accumulate);
  ...
  // COMPUTE MEDIAN
  double medianVal = -1; // TODO: Fix uninitialized.
  for (int i = 0; i <= n - 1; i++) {
...
void StarSolver::get_centroids() {
  ...
  case GLOBAL_MEDIAN: {
    ...
    filter_buffer =
        img_median * cv::Mat::ones(cur_img.rows, cur_img.cols, cur_img.type());
    break;
  }
  case GLOBAL_MEAN: {
    filter_buffer = cv::mean(cur_img) *
                    cv::Mat::ones(cur_img.rows, cur_img.cols, cur_img.type());
    break;
  }
  default: {
  ...
  if (img_threshold < 0) {
    ...
    case GLOBAL_MEDIAN_ABS: {
      cur_img.convertTo(filter_buffer, CV_32F);
      filter_buffer = cv::abs(filter_buffer);
      double img_median = get_median(
          filter_buffer, (int)(filter_buffer.cols * filter_buffer.rows));
      filter_buffer =
          med_sigma_coef * img_median *
          cv::Mat::ones(filter_buffer.rows, filter_buffer.cols, cur_img.type());
      break;
    }
  ...
  cv::Mat element;
  if (binary_close) {
    // element = cv::getStructuringElement(morph_elem, cv::Size(2 * morph_size +
    // 1, 2 * morph_size + 1), cv::Point(morph_size, morph_size));
    element = cv::getStructuringElement(cv::MORPH_ELLIPSE,
                                        cv::Size(morph_size, morph_size));
    cv::morphologyEx(thresh_img, thresh_img, cv::MORPH_CLOSE, element);
    // element = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(2 *
...
void StarSolver::findContours() {
  cv::findContours(cur_img, contours, cv::RETR_EXTERNAL,
...
void StarSolver::computeMoments() {
  ...
  for (long unsigned int ii = 0; ii < contours.size(); ii++) {
    ...
    // major = np.sqrt(2 * (m2_xx + m2_yy + np.sqrt((m2_xx - m2_yy)**2 + 4 *
    // m2_xy**2))) minor = np.sqrt(2 * max(0, m2_xx + m2_yy - np.sqrt((m2_xx -
    // m2_yy)**2 + 4 * m2_xy**2)))
